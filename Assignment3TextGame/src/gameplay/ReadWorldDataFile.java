...
public class ReadWorldDataFile {
	...
	{	
		...
		case treasurechest:					
			...
			return new TreasureChest("TreasureChest", v);
		case warchest:
			Wieldable w = (Wieldable) parsePickUpData(tokens[2]);
			return new WarChest("WarChest", w); 
	...
	private static World readFile(String levelDataFile) throws IOException
	{
		...
		{			 			
			...
			do
			{
				...
				case connect:
					...
					rooms[i].setConnectingRooms(connectingRooms);
					break;
				case startroom:
					startRoomIndex = i;
					break;
				case description:
					...
					log(description);
					rooms[i] = new Room(description);
					break;
				case monster:
					//read and print the meta tag @numberofrooms 
					String monsterText = fileScan.nextLine();
					rooms[i].setMonster(parseMonsterData(monsterText));
					break;
				case numberofpickups:
					...
					rooms[i].setPickupsInRoom(pickups);
		...
		fileScan.close();
		return new World(rooms, startRoomIndex);
